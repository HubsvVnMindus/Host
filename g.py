...
def banner(game):
    ...
    draw_line()
    prints(7, 205, 240, f" 👤 Admin: HUNGTOOL | 📺 YouTube: @Tool-Xworld ".center(52))
    draw_line()
...
def load_config_vth():
    ...
    if status == 'y':
        ...
        # <-- ĐÃ SỬA THỤT LỀ ĐÚNG CHO TOÀN BỘ KHỐI NÀY
        conf.update({
            'coins1': float(input(' -> Mức cược tối thiểu: ')),
            'coins2': float(input(' -> Mức cược tối đa: ')),
            ...
            'up_bet': -1 * float(input(' -> Lỗ bao nhiêu thì tăng cược: ')),
            'up_bet2': float(input(' -> Mức cược tăng thêm: '))
        })
    else:
        conf.update({
            'coins1': 0,
...
def top10_vth(s, headers, Coin):
    try:
        ...
        return top10_vth(s, headers, Coin)
...
def bet_vth(s, headers, room_id, config, stats):
    ...
    if profit <= config['stop_bet2']:
        prints(255, 50, 50, " [!] Đã chạm ngưỡng cắt lỗ. Dừng tool.")
        sys.exit()
    ...
    if config['status_bet'] == 'y' and config['start_bet'] <= stats['streak'] <= config['end_bet']:
        if profit <= config['up_bet']:
            ...
            if config['Coin'] == 'BUILD':
                step = 100 if c2 >= 100 else 1
                bet_amount = round(random.randint(int(c1), int(c2)) / step) * step
            else:
        ...
        try:
            ...
            if res['code'] == 0:
                ...
                prints(255, 50, 50, f" ❌ Lỗi: {res.get('msg', 'Unknown')}")
...
def main_vth():
    ...
    stats = {
        ...
        'asset_0': asset_init[config['Coin']]
    }
        ...
        while True:
            ...
            if check_d10[0][0] == d10[0][0] + 1:
                ...
                if profit >= config['stop_bet']:
                    prints(0, 255, 0, " [!] Đã đạt mục tiêu chốt lời. Nghỉ ngơi thôi!")
                    sys.exit()
